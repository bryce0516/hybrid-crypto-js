        ...
        _createClass(Crypt, [
            ...
            },
            {
                ...
                value: function verify(publicKey, _signature, decrypted) {
                    ...
                    var _this$_parseSignature = this._parseSignature(
                            _signature,
                        ),
                    ...
                    if (typeof publicKey === 'string')
                        publicKey = pki.publicKeyFromPem(publicKey); // Verify signature
            ...
            },
            {
                ...
                value: function encrypt(publicKeys, message, signature) {
                    ...
                    var key = forge.random.getBytesSync(
                        this.options.aesKeySize / 8,
                    ); // Encrypt random key with all of the public keys

                    var encryptedKeys = {};
                    publicKeys.forEach(function(publicKey) {
                        var encryptedKey = publicKey.encrypt(
                            key,
                            _this.options.rsaStandard,
                        );
                    ...
                    payload.tag =
                        cipher.mode.tag &&
                        forge.util.encode64(cipher.mode.tag.getBytes()); // Return encrypted message
            ...
            },
            {
                ...
                value: function decrypt(privateKey, encrypted) {
                    ...
                    if (typeof privateKey === 'string')
                        privateKey = pki.privateKeyFromPem(privateKey); // Get key fingerprint
                    ...
                    if (!encryptedKey)
                        throw "RSA fingerprint doesn't match with any of the encrypted message's fingerprints"; // Get bytes of encrypted AES key, initialization vector and cipher
            ...
            },
            {
                ...
                value: function encryptRsa(publicKey, message) {
                    const inputedPublicKey = forge.pki.publicKeyFromPem(
                        publicKey,
                    );
